...
class Database:
    ...
    print(f"Итоговый DATABASE_URL: {self.database_url}")
    ...
    if self.database_url:
        ...
        if 'postgres' in self.database_url.lower():
            ...
            print(f"✅ Определено: PostgreSQL - {self.database_url[:50]}...")
        else:
            print(f"ℹ️ Определено: SQLite (путь: {self.database_url})")
    else:
    ...
    if self.is_postgres and self.database_url:
        ...
        try:
            print(f"🔗 Подключаемся к PostgreSQL: {self.database_url[:50]}...")
            
            # Парсим URL
            if self.database_url.startswith('postgresql://'):
                conn = psycopg2.connect(self.database_url, sslmode='require')
            else:
                # Ручной парсинг для старых форматов
                import re
                ...
                if match:
                    ...
                    raise ValueError(f"Неверный формат URL: {self.database_url}")
