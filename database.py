...
class Database:
    ...
    print("=" * 50)
    print(f"DATABASE_URL получен: {self.database_url}")
    ...
    if self.database_url:
        ...
        if self.database_url.startswith('postgres://') or self.database_url.startswith('postgresql://'):
            ...
            print("✅ Определено: PostgreSQL")
        else:
            print("ℹ️ Определено: SQLite (по умолчанию)")
    else:
        print("⚠️ DATABASE_URL не найден, используем SQLite по умолчанию")
