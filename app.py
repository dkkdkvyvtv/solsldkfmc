...
def get_or_create_user(telegram_user_data):
    ...
    try:
        ...
        db.execute_query(cursor, 'SELECT id, balance FROM users WHERE telegram_id = ?', (telegram_user_data['id'],))
        user = db.fetchone(cursor)
...
def api_admin_categories_add():
    ...
    try:
        ...
        # Добавляем категорию
        db.execute_query(cursor, '''@app.route('/api/admin/categories/exists', methods=['POST'])
@require_api_key
def api_admin_categories_exists():
    """Проверка существования и активности категории"""
    try:
...
def api_user_profile():
    ...
    try:
        ...
        # Получаем ID пользователя в базе
        db.execute_query(cursor, 'SELECT id, balance, first_name, username, photo_url FROM users WHERE telegram_id = ?', (user_id,))
        user = db.fetchone(cursor)
