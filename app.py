...
def api_admin_verify_user():
    ...
    try:
        ...
        # Выполняем действие
        is_verified_val = db.bool_to_sql(action == 'verify')
        
        db.execute_query(cursor, 'UPDATE users SET is_verified = ? WHERE id = ?', (is_verified_val, user_db_id))
...
def api_admin_categories_add():
    ...
    try:
        ...
        db.execute_query(cursor, '''
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (data['name'], data['display_name'], data['icon'], section_id, data['sort_order'], db.bool_to_sql(True)))
...
def api_admin_products_add():
    ...
    try:
        ...
        db.execute_query(cursor, '''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (name, description, price, image_path, category, specifications, db.bool_to_sql(True)))
...
def api_admin_locations_add():
    ...
    try:
        ...
        db.execute_query(cursor, '''
            INSERT INTO pickup_locations (name, address, city, location_type, delivery_price, is_active)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (data['name'], data['address'], data['city'], data['location_type'], delivery_price, db.bool_to_sql(True)))
...
# Добавь в app.py endpoint для фиксации:
@app.route('/api/fix-inactive')
def api_fix_inactive():
    """Активация всех неактивных записей"""
    try:
        conn = db.get_connection()
        cursor = conn.cursor()
        
        # Активируем все разделы
        db.execute_query(cursor, 'UPDATE sections SET is_active = 1 WHERE is_active = 0 OR is_active IS NULL')
        
        # Активируем все категории  
        db.execute_query(cursor, 'UPDATE categories SET is_active = 1 WHERE is_active = 0 OR is_active IS NULL')
        
        # Активируем все товары
        db.execute_query(cursor, 'UPDATE products SET is_active = 1 WHERE is_active = 0 OR is_active IS NULL')
        
        conn.commit()
        conn.close()
        
        return jsonify({'success': True, 'message': 'Все записи активированы'})
    except Exception as e:
...
@app.route('/api/admin/users/verified', methods=['GET'])
@require_api_key
def api_admin_users_verified():
    ...
    try:
        ...
        print(f"Ошибка получения верифицированных пользователей: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500
...
if __name__ == '__main__':
    ...
    # Получаем порт из окружения Render или используем 5000 по умолчанию
